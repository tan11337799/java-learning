...
在我们编写实体类的时候无论多少个属性，都要为其提供 getter 和 setter 方法，如果属性过多就会显得代码过于臃肿，这个时候可以使用@Getter/@Setter来代替 getter 和 setter 方法。
...
@EqualsAndHashCode 注解就是用于根据类所拥有的非静态字段自动重写 [equals](https://so.csdn.net/so/search?q=equals&spm=1001.2101.3001.7020) 方法和 hashCode 方法,方便我们用于对象间的比较。
