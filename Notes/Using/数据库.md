...
为root用户分配权限：`GRANT ALL PRIVILEGES ON *.* TO 'root'@'%' WITH GRANT OPTION;`
...
* 如果设置免密登录（在etc/my.cnf中设置了skip-grant-table属性），则服务器会启用`--skip-networking`以阻止远程连接，这将导致其他服务器无法通过3306端口连接该数据库。

  解决方案：

  * 删除配置文件中的skip-grant-tables属性；
...
* 在测试类中直接注入mapper对象，通过对象调用BaseMapper中的方法。
  ...
  > 这里selectList为继承自BaseMapper的方法：
  >
  > `List<T> selectList(@Param(Constants.WRAPPER) Wrapper<T> queryWrapper);`
  >
  > 需要传入一个条件构造器对象，如果不定义条件则传入null。



### 注解

当我们不设定关联注解时，默认查询语句的表名为实体类名，默认查询/条件中的字段为实体类中的属性名。

如果需要自定义查询的表名、字段名需要使用@TableName，@TableId注解。



##### @TableName

该注解用于实现实体类型和数据库中表的映射关系。

属性：

* value：指定需要关联的表名；
* autoResultMap：是否需要自动构造resultMap并使用
* resultMap：选择xml中自定义的resultMap



##### @TableId

指定实体类属性和表中的主键字段对应。（在其对照的mysql语句会使用AS进行别名修改）
...
* type：设置主键生成策略。
  ...
  常用策略：（1）IdType.ASSIGN_ID（默认，选择基于雪花算法的生成策略）；（2）IdType.AUTO（使用数据库的自增策略，需要保证数据库设置递增）
...
* value：用于指定作为非主键的字段；
* update：用于预处理set字段，自定义注入（如update="%s+1"，则插入到数据库的值变为原来的值+1）
* condition：预处理where实体条件自定义的运算规则（如conditon=sqlCondition.LIKE，则会对该字段进行like的匹配）
* fill：在进行插入、更新等操作时，对该字段进行自动填充。语法：fill=FieldFill.xxx，xxx如下：
  * DEFAULT：默认不处理
  * INSERT：插入时填充字段
  * UPDATE：更新时填充字段
...
用于标识逻辑删除。逻辑删除将原先的删除操作转变为修改操作（将逻辑删除的字段值修改），查询功能只会查询未被逻辑删除的数据。

> 在mysql中对数据的删除可以分为物理删除和逻辑删除（用于数据恢复）。
>
> 物理删除是真删除，将对应数据从数据库删除；
>
> 逻辑删除是假删除，将对应数据中代表是否被删除字段的状态改为“被删除状态”，但数据库仍然可以看到该数据。
...
* `insert(T entity)`     参数为数据库表映射的实体类，实现插入记录

  > 在添加实体时，如果不设定id，则会默认使用通过雪花算法获取的id。

**修改**

- `update(@Param(“et”) T entity, @Param(“ew”) Wrapper wrapper)`     参数前面是实体类，后面是查询的条件，实现更新记录

  对应SQL语句：UPDATE user SET username=?, password=? WHERE id=?

**删除**

* `deleteById(Serializable id) `    参数为id，实现删除记录。

  对应SQL语句：DELETE FROM user WHERE id = ?

* `deleteByMap(@Param("cm") Map<String, Object> columnMap)`    根据map中的多个条件删除数据

  对应SQL语句：DELETE FROM user WHERE id = ? AND username = ?
...
* `deleteBatchIds(@Param("coll") Collection<?> idList)`    利用列表等集合批量删除数据

  对应SQL语句：DELETE FROM user WHERE id IN ( ? , ? , ? )
...
* `selectById(Serializable id)`  根据id查询对应数据

  对应SQL语句：SELECT * FROM user WHERE id=?

* `selectBatchIds(@Param(Constants.COLLECTION) Collection<? extends Serializable> idList)` 根据数据集合查询数据

  对应SQL语句：SELECT id,username,password FROM user WHERE id IN ( ? , ? , ? )

* `selectByMap(@Param("cm") Map<String, Object> columnMap)`    根据map中的多个条件查询数据

  对应SQL语句：SELECT id,username,password FROM user WHERE username= ? AND password = ?

* `selectList(@Param(“ew”) Wrapper wrapper) `    参数为查找条件，实现查找出所有符合条件的记录

  对应SQL语句：按条件创建

* `selectPage(RowBounds rowBounds, @Param(“ew”) Wrapper wrapper)`    参数前面写Page类对象就行，后面为查询条件，实现分页查找



### 通用Service

继承`BaseMapper<T>` 后，无需编写 `mapper.xml` 文件，即可获得CRUD功能。而IService则是对BaseMapper的扩展，IService 的默认实现com.baomidou.mybatisplus.extension.service.impl.ServiceImpl 就是调用BaseMapper 来操作数据库。

我们使用mybatis时，只需要使用service提供的扩展方法即可，如果service方法不足以满足我们的需要，则可以进行添加。

##### 实现流程

（1）创建UserService接口，继承`IService<T>`接口，T为需要操作的实体类类型；

（2）创建UserServiceImpl实现类，实现UserService接口，并继承`ServiceImpl<M extends BaseMapper<T>, T>`类，第一个泛型为service类对应的mapper接口，第二个泛型为需要操作的实体类类型。

（3）在UserServiceImpl上，添加@Service注解，将该类交给Spring管理。

##### **常用方法**

**添加（返回是否添加成功的boolean值）：**

添加单条数据到数据库中：`boolean save(T entity)`

批量添加数据到数据库中：`boolean saveBatch(Collection<T> entityList)`

**删除（返回是否删除成功的boolean值）：**

根据queryWrapper删除若干记录：`remove(Wrapper<T> queryWrapper)`

根据id删除单条记录：`boolean removeById(Serializable id)`

根据id列表删除多条记录：`boolean removeByIds(Collection<? extends Serializable> idList)`

**修改：**

根据 UpdateWrapper更新记录：`boolean update(Wrapper<T> updateWrapper)`

根据传入实体中的id修改单条记录：`boolean updateById(T entity)`

**查询：**

查询总记录数：`int count(Wrapper<T> queryWrapper)`

查询单条记录：`T getOne(Wrapper<T> queryWrapper)`	用某实体对象接收

根据queryWrapper查询若干记录：`List<T> list(Wrapper<T> queryWrapper)`

根据queryWrapper进行分页查询：`<E extends IPage<T>> E page(E page, Wrapper<T> queryWrapper)`
...
![1657077378476](/1657077378476.png)

Wrapper是条件构造器的顶级父类，其子类AbstractWrapper用于查询条件的封装，并生成sql的where条件。

其子类包括：

* QueryWrapper：用于查询的条件构造器；
* UpdateWrapper：用于修改的条件构造器；
* AbstractLambdaWrapper：使用Lambda的条件构造器；



##### QueryWrapper

QueryWrapper可以实现对数据库的增删改查条件判断。

**使用：**

```java
//创建查询条件构造器（在泛型中输入需要查询的实体类）
QueryWrapper<User> userQueryWrapper = new QueryWrapper<User>();
//封装条件，连用多个方法，各个方法之间用AND连接
userQueryWrapper.like("username","a").isNotNull("password");
//（以查询为例）根据查询条件构造器获取查询结果
List<User> users = userMapper.selectList(userQueryWrapper);
```

**涉及方法：**

`select(col1,col2,.…)`  选择查询的字段

`between(col,val1,val2)` 判断某字段是否在val1,val2范围内

`isNotNull(col)`   判断某字段是否为空

`orderByDesc(col)`   按col字段降序排列

`orderByAsc(col)`  按col字段升序排列 



**组装查询条件**

要求：查询名字里包含a字母、年龄在20~30之间、email不为空的所有值
...
要求：查询用户信息，按照年龄的降序排序，若年龄相同，则按照id升序排序
...
**注意点：**可以发现，如果添加了逻辑删除字段，则mybatis会将内部的删除功能变成修改功能。



**组装修改条件**

要求：修改(年龄大于20并且用户名中包含有a)或邮箱为null的用户信息
...
UpdateWrapper封装类一般用于修改数据的条件判断。

与QueryWrapper相比，UpdateWrapper可以使用类似QueryWrapper的条件判断功能，还可以用set()方法设定满足条件时各字段需要修改的值。

使用UpdateWrapper不需要创建实体对象，update方法的第一个参数可以设为null。
...
为了防止字段名出错，LambdaQueryWrapper可以通过实体类::getter的方式获取实体类属性对应的字段值。
...
LambdaQueryWrapper可以通过实体类::getter()的方式获取实体类属性对应的字段值实现修改操作。

注意点：需要在条件判断和set两个位置使用函数式接口方式获取字段名。
...
（2）在测试类中创建Page对象，使用selectPage方法进行分页操作。

可以通过page对象获取page的相关内容。
...
第一个参数page是MP提供的分页对象，第二个参数为需要根据哪一个参数查询信息
...
**背景：**数据库的扩展方式包括：业务分库、主从复制（主从服务器数据保持一致，读写分离）、数据库分表

* 数据库分表：将不同业务数据分散存储到不同的数据库服务器中，如果业务发展迅速，同一业务的单表数据会达到单表数据库服务器的处理瓶颈，因此需要将单表数据进行拆分。

  方式：垂直分表（对字段进行拆分）、水平分表（将完整的数据分到多个表中）

  水平分表就可以根据雪花算法的主键递增策略实现。

**原理：**

MP的默认主键策略是基于雪花算法的自增主键，因此不需要对主键进行自增配置。

SnowFlake 算法，是 Twitter 开源的分布式 id 生成算法。其核心思想就是：使用一个 64 bit 的 long 型的数字作为全局唯一id。在分布式系统中的应用十分广泛，且ID 引入了时间戳，基本上保持自增的，能够保证不同表的主键的不重复性，以及相同表的主键的有序性。
...
- 主键采用雪花算法生成值的前提是实体类的主键属性名称必须为id，如果需要使用别的字段作为主键，则需要使用@TableId注解；
- 在设计表时，id设为bigint类型；
- 在设置实体类时，id设为long类型。
...
MP不会对mybatis的功能进行修改，而是只进行增强。

实现步骤：

（1）在resources下创建mapper包，在包中创建mapper.xml文件。

（2）和mybatis的方式配置SQL语句；

（3）在原先的Mapper接口中，添加自定义的方法。



##### Condition条件判断

组装方法存在多个重载。可以在第一个参数位置添加condition进行条件判断。

因此可以将条件判断方法写入到Wrapper的组装条件中，表示满足条件时才组装该条件。

> 常用：`StringUtils.isNotBlank()`    判断某个字符串是否为空字符串，null，空白符

要求：username不为空时选择username是否存在a，age上下限不为null时选择age是否在30以内。
...
Redis是是完全开源的，遵守 BSD 协议，是一个基于内存的k-v结构数据库，具有很高的读写性能，适合存储热点数据。

Redis被称为结构化的NoSql数据库，也就是非关系型数据库

> 关系型数据库指表与表之间存在关系，包括mysql、oracle、DB2、SQLServer
>
> 非关系数据库是关系型数据库的补充，包括Redis、Mongo db
